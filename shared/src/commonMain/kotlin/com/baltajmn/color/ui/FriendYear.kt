...
@Composable
private fun Overlay(title: String, onClose: () -> Unit, content: @Composable ColumnScope.() -> Unit) {
    Column(
...
fun FriendYear(person: Profile, today: LocalDate, onClose: () -> Unit) {
    ...
    val days = rows.orEmpty().associate { it.day to it.color }
    ...
    Overlay(person.displayName, onClose) {
...
@Composable
fun FriendDay(row: FeedRow, author: String, onClose: () -> Unit, onPhoto: (ImageBitmap) -> Unit) {
    val photo by produceState<ImageBitmap?>(null, row.cacheName) { value = Friends.photo(row) }
    Overlay(S.longDateWithYear(row.date), onClose) {
        Spacer(Modifier.height(8.dp))
        ChromaCard(row.entry(), row.date, author = author, photo = photo, onPhoto = onPhoto)
    }
}
